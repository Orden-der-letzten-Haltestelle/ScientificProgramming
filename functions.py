...
class MFunc(ABC, Plottable):
    ...
    def obersumme(self, start, end, n = 10):
        ...
        if len(nullstellen) == 0:
            return self._obersumme(start, end, n)

        for i, ele in enumerate(nullstellen):
            if ele != start and i == 0:
                sum += self._obersumme(start, ele, n)

            if i < len(nullstellen)-1:
                sum += self._obersumme(ele, nullstellen[i+1], n)
            else: 
                sum += self._obersumme(ele, end, n)
    ...
    def trapezregel(self, start, end, n = 10):
        ...
        if len(nullstellen) == 0:
            return self._trapezregel(start, end, n)

        for i, ele in enumerate(nullstellen):
            if ele != start and i == 0:
                sum += self._trapezregel(start, ele, n)
            
            if i < len(nullstellen)-1:
                sum += self._trapezregel(ele, nullstellen[i+1], n)
            else: 
                sum += self._trapezregel(ele, end, n)
